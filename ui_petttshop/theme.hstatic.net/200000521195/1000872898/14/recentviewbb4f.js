...
function getProduct(alias) {
	...
	Haravan.getProduct(alias,function(product){
	//	console.log(product);
		...
		var pt ="";
		 if (product.price < 1){
			 ...
			 var price = '<div class="special-price"><span class="price product-price">'+  Haravan.formatMoney(product.price_min, "{{amount_no_decimals_with_comma_separator}}₫" ) +'</span> </div>';
		 ...
		 if(product.images.length == 0){
			 var img = '<img src="//haravan.com/thumb/medium/assets/themes_support/noimage.gif" alt="'+product.title+'">'
			 }else{
				 var img = '<img src="'+product.featured_image+'" alt="'+product.title+'">'
				 }
		  $('<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">'
			+'<div class="product_border">'
			+'<div class="product-loop-1 product-loop-2 product-base">'
			+'<div class="product-thumbnail flexbox-grid relative">'
			+'<a class="image_link display_flex" title="'+product.title+'" href="'+product.url+'">'
			+ img
			+'</a>'
			+ pt
